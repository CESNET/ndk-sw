...
int dpdk_receive_loop(void *params);
...
int _dpdk_receive_parse_app_opt(int argc, char **argv);
...
int dpdk_receive_run_single(struct ndp_tool_params *p)
{
	...
	if (!p->pcap_file) {
		fprintf(stderr, "initializing PCAP file '%s' failed\n", p->pcap_filename);
		goto launch_fail;
	}

	gettimeofday(&p->si.startTime, NULL);
	RTE_LCORE_FOREACH_WORKER(core_id) {
		ret = rte_eal_remote_launch(dpdk_receive_loop, p, core_id);
		if (ret < 0) {
...
void *dpdk_receive_run_thread(void *tmp)
{
	...
	int core_id;
	int ret;
	char *temp_file_names[p->mode.dpdk.queue_count];
	...
	for (thread_counter = 0; thread_counter < p->mode.dpdk.queue_count; thread_counter++) {
		temp_file_names[thread_counter] = malloc(strlen(p->pcap_filename) + 8);
		if (!temp_file_names[thread_counter]) {
			goto alloc_fail;
		}
		sprintf(temp_file_names[thread_counter], "%s.%d", thread_data[thread_counter]->params.pcap_filename, thread_data[thread_counter]->params.queue_index);
		thread_data[thread_counter]->params.pcap_filename = temp_file_names[thread_counter];
		thread_data[thread_counter]->params.pcap_file = pcap_write_begin(thread_data[thread_counter]->params.pcap_filename);
		if (!thread_data[thread_counter]->params.pcap_file) {
			fprintf(stderr, "initializing PCAP file '%s' failed\n", thread_data[thread_counter]->params.pcap_filename);
			goto file_fail;
		} 
	}
	...
	RTE_LCORE_FOREACH_WORKER(core_id) {
		while (thread_counter < p->mode.dpdk.queue_count) {
			if (thread_data[thread_counter]->params.queue_index < 0) { 	// This is to prevent negative queue numbers crashing the program
				thread_counter++;
				continue;
			}
			thread_data[thread_counter]->state = TS_RUNNING;
			thread_data[thread_counter]->params.update_stats = update_stats_thread;
			thread_data[thread_counter]->params.si.progress_letter = 'R';
			gettimeofday(&thread_data[thread_counter]->params.si.startTime, NULL);
			ret = rte_eal_remote_launch(dpdk_receive_loop, &thread_data[thread_counter]->params, core_id);
...
file_fail:
	for (thread_counter = 0; thread_counter < p->mode.dpdk.queue_count && thread_data[thread_counter]->params.pcap_file; thread_counter++) {
		fclose(thread_data[thread_counter]->params.pcap_file);
	}
alloc_fail:
	for (thread_counter = 0; thread_counter < p->mode.dpdk.queue_count && thread_data[thread_counter]->params.pcap_filename; thread_counter++) {
		free(temp_file_names[thread_counter]);
	}
	for (thread_counter = 0; thread_counter < p->mode.dpdk.queue_count; thread_counter++) {
...
int dpdk_receive_loop(void *params)
{
	...
	while (!stop) {
		...
		cnt = rte_eth_rx_burst(port_id, queue_id, packets, brst_size);
		...
		ret = pcap_write_packet_burst(statpackets, cnt, p->pcap_file, p->mode.dpdk.ts_mode, p->mode.dpdk.trim);
		if (ret) {
			return ret;
		}
	}
...
{
	int ret;
	p->mode.dpdk.ts_mode = TS_MODE_NONE;
	p->mode.dpdk.trim = (unsigned)-1;
	wordexp_t *args = &p->mode.dpdk.args;
	ret = wordexp("DPDK_RECEIVE", args, 0);
	return ret;
}
...
int dpdk_receive_check(struct ndp_tool_params *p)
{
	...
	if (argc > 1) {
		ret = _dpdk_receive_parse_app_opt(argc, app_argv);
		if (ret) {
			fprintf(stderr, "_dpdk_receive_parse_app_opt failed: %d\n", ret);
			goto eal_fail;	
		}
	}
	...
	RTE_ETH_FOREACH_DEV(port_id) {
		...
		for (queue_id = 0; queue_id < dev_info.max_rx_queues && queue_idx < dpdk_params->queues_available; queue_id++, queue_idx++) {
			...
			if (list_range_contains(queue_range, queue_idx) && pools_assigned < rte_lcore_count() - 1) {
				if (!use_multipool) {
					...
					sprintf(pool_name_buff, "rxpool%d", queue_idx);
	...
	RTE_ETH_FOREACH_DEV(port_id) {
		...
		for (queue_id = 0; queue_id < dev_info.max_rx_queues; queue_id++, queue_idx++) {
	...
	int option_index __attribute__((unused)))
{
	...
	case 't':
		if (strcmp("system", optarg) == 0) {
			p->mode.dpdk.ts_mode = TS_MODE_SYSTEM;
		} else if (sscanf(optarg, "header:%d", &p->mode.dpdk.ts_mode) == 1) {
			if (p->mode.dpdk.ts_mode < 0) {
		...
		break;
	case 'r':
		if (sscanf(optarg, "%u", &p->mode.dpdk.trim) != 1) {
...
int _dpdk_receive_parse_app_opt(int argc, char **argv) {
	...
	optind = 1;
	while (true) {
		ret = getopt_long(argc, argv, "s:p:r:c:m", long_opts, &opt_index);
		if (ret == -1)
			break;
