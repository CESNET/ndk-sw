...
int dpdk_read_loop(void *params);
...
int _dpdk_read_parse_app_opt(int argc, char **argv);
...
int dpdk_read_run_single(struct ndp_tool_params *p)
{
	...
	RTE_LCORE_FOREACH_WORKER(core_id) {
		ret = rte_eal_remote_launch(dpdk_read_loop, p, core_id);
		if (ret < 0) {
...
void *dpdk_read_run_thread(void *tmp)
{
	...
	RTE_LCORE_FOREACH_WORKER(core_id) {
		while (thread_counter < p->mode.dpdk.queue_count) {
			...
			ret = rte_eal_remote_launch(dpdk_read_loop, &thread_data[thread_counter]->params, core_id);
...
int dpdk_read_check(struct ndp_tool_params *p)
{
	...
	if (argc > 1) {
		ret = _dpdk_read_parse_app_opt(argc, argv_app);
		if (ret) {
			fprintf(stderr, "_dpdk_read_parse_app_opt failed: %d\n", ret);
			goto eal_fail;	
		}
	}
