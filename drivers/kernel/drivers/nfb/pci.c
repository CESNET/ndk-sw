...
static int nfb_fdt_fixups(struct nfb_device *nfb)
{
	...
	if (!strcmp(name, "NFB-200G2QL")) {
		...
		if (!strcmp(card_name, "IA-420F") || !strcmp(card_name, "TERASIC-A2700")) {
			...
			//nfb_fdt_create_boot_type(fdt, node, "BPI", 16);
		} else if (!strcmp(card_name, "N6010") || !strncmp(card_name, "FB2CDG1", 7)) {
			...
			prop32 = cpu_to_fdt32(1);
			fdt_appendprop(fdt, node, "num_flash", &prop32, sizeof(prop32));
			prop32 = cpu_to_fdt32(64 * 1024 * 1024);
			fdt_appendprop(fdt, node, "mtd_size", &prop32, sizeof(prop32));

			nfb_fdt_create_binary_slot(fdt, node, "image0", "application0" , 0, 0, 0, 0x00000000, 0x04000000);
